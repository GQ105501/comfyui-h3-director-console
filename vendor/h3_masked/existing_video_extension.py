...
def _apply_video_context_feather(video_mask, video_steps, feather_tokens):
    """Release the first generated video tokens with a half-cosine ramp.

    The inherited prefix remains exactly protected (mask=0). The following
    tokens transition from mostly inherited to mostly generated, avoiding the
    0->1 temporal mask edge that can cause a camera-scale jump at the first
    delivered frame.
    """
    video_steps = int(video_steps)
    available = max(0, int(video_mask.shape[2]) - video_steps)
    feather = max(0, min(int(feather_tokens), available))
    video_mask[:, :, :video_steps] = 0.0
    if feather > 0:
        i = torch.arange(
            1, feather + 1, device=video_mask.device, dtype=video_mask.dtype
        )
        ramp = 0.5 - 0.5 * torch.cos(torch.pi * i / float(feather + 1))
        shape = [1] * video_mask.ndim
        shape[2] = feather
        video_mask[:, :, video_steps:video_steps + feather] = ramp.view(*shape)
    return feather


def _seed_video_context_feather(
    out_video,
    target_video,
    source_video,
    video_steps,
    ...
    """Blend transition-token seeds from the inherited tail to the target.

    Repeating the last inherited token across the whole feather region merely
    postpones the hard boundary. Matching the mask's half-cosine ramp gives
    every transition token a progressively more target-like seed, so the VAE
    sees a temporal bridge instead of a frozen run followed by a reset.
    """
    video_steps = int(video_steps)
    available = max(0, int(out_video.shape[2]) - video_steps)
    feather = max(0, min(int(feather_tokens), available))
    if feather == 0:
        return 0

    i = torch.arange(1, feather + 1, device=out_video.device, dtype=out_video.dtype)
    ramp = 0.5 - 0.5 * torch.cos(torch.pi * i / float(feather + 1))
    ramp = ramp.view(1, 1, feather, 1, 1)
    inherited = source_video[:, :, -1:].to(
        device=out_video.device, dtype=out_video.dtype
    ).expand(-1, -1, feather, -1, -1)
    fresh = target_video[:, :, video_steps:video_steps + feather].to(
        device=out_video.device, dtype=out_video.dtype
    )
    out_video[:, :, video_steps:video_steps + feather] = torch.lerp(
        inherited, fresh, ramp
    )
    return feather
...
class MiniMaxH3GeneratedAVMaskedContext:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "video_feather_tokens": ("INT", {
                    "default": 3, "min": 0, "max": 16,
                    "tooltip": "Half-cosine release across the first generated video tokens. 3 reduces camera-scale jumps while preserving the exact inherited prefix."
                }),
            }
        }
    ...
    DESCRIPTION = (
        ...
        "new target prefix and protected with AV noise-mask 0. A short seeded "
        "video feather releases the future region gradually."
    )
    ...
    def prepare(
        ...
        # Seed the partially denoised tokens with the same gradual release as
        # their masks. This avoids both an immediate reset and a delayed reset
        # after a frozen run of repeated source tokens.
        video_feather = _seed_video_context_feather(
            out_video,
            target_video,
            source_video,
            video_steps,
            video_feather_tokens,
        )
        ...
        video_feather = _apply_video_context_feather(
            video_mask, video_steps, video_feather
        )
        _apply_audio_context_feather(audio_mask, audio_steps, audio_feather_ticks)
        ...
        _LOG.info(
            "h3_masked_extension: latent->latent AV continuation protected %d frames = %d video steps / %d audio steps; video feather %d tokens; audio feather %d ticks; target %d frames",
            n, video_steps, audio_steps, video_feather,
            max(0, min(int(audio_feather_ticks), audio_steps)), target_frames,
        )
        return (out, n)
